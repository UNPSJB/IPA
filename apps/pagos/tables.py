...
class CobrosTable(tables.Table):
    ...
    es_por_canon = tables.BooleanColumn(verbose_name="Tipo",yesno="Canon,Infraccion")
...
class PagosTable(tables.Table):
    #fecha = tables.DateColumn(short=False,verbose_name="Fecha de Pago", attrs={"td": {"id": lambda record: "fecha-"+str(record.id)+"-"+record.fecha.strftime("%d/%m/%Y")}})
    fecha = tables.DateColumn(short=False,verbose_name="Fecha de Pago", attrs={"td": {"id": lambda record: "fecha-"+str(record.id)}})
    #monto = tables.Column(verbose_name="Monto ($)", attrs={"td": {"id": lambda record: "monto-"+str(record.id)+"-"+str(record.monto)}})
    monto = tables.Column(verbose_name="Monto ($)", attrs={"td": {"id": lambda record: "monto-"+str(record.id)}})
    #es_por_canon = tables.BooleanColumn(verbose_name="Tipo",yesno="Canon,Infraccion", attrs={"td": {"id": lambda record: "tipo-"+str(record.id)+"-"+str(record.tipo_de_pago())}})
